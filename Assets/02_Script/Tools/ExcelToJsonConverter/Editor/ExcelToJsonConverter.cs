...
{	
	...
	{
        if (false == System.IO.Directory.Exists(inputPath))
        {
            Debug.LogErrorFormat("[LSH] Error!! not exist for Excel file inputpath\ninputPath : {0}", inputPath);
            return;
        }
	...
	private string GetSpreadSheetJson(DataSet excelDataSet, string sheetName)
	{
        ...
        {
            JsonData pRowJsonData = new JsonData();
            for (int iCol = 0; iCol < dataTable.Columns.Count; ++iCol)
            {
                var strColumnName = dataTable.Columns[iCol].ColumnName;
                var strValue      = dataTable.Rows[iRow][iCol].ToString();

                if (strColumnName.ToLower().Contains("i_"))
                {
                    int iValue;
                    if (false == int.TryParse(strValue, out iValue))
                    {
                        Debug.LogErrorFormat("[LSH] Error!! type error ColumnName : {0}, Row : {1}, Value : {2}",
                            strColumnName, iRow, strValue);
                    }
                    pRowJsonData[strColumnName] = iValue;
                }
                else if (strColumnName.ToLower().Contains("f_"))
                {
                    float fValue;
                    if (false == float.TryParse(strValue, out fValue))
                    {
                        Debug.LogErrorFormat("[LSH] Error!! type error ColumnName : {0}, Row : {1}, Value : {2}",
                            strColumnName, iRow, strValue);
                    }
                    pRowJsonData[strColumnName] = fValue;
                }
                else if (strColumnName.ToLower().Contains("b_"))
                {
                    bool bValue;
                    if (false == bool.TryParse(strValue, out bValue))
                    {
                        Debug.LogErrorFormat("[LSH] Error!! type error ColumnName : {0}, Row : {1}, Value : {2}",
                            strColumnName, iRow, strValue);
                    }
                    pRowJsonData[strColumnName] = bValue;
                }
                else
                {
                    pRowJsonData[strColumnName] = strValue;
                }
            }
            pJsonData.Add(pRowJsonData);
        }
